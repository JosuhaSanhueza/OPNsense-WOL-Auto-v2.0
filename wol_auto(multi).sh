...
# === VALIDACIÓN DE RANGOS (MULTI-LAB) ===
es_ip_valida() {
    ip=$1
...
if [ "$es_feriado" -gt 0 ]; then
    ...
    enviar_mensaje "🚫 Hoy $hoy_legible es feriado. No se encienden LAB11 y LAB12."
    enviar_log
    rm -f "$LOG_FILE"
    exit 0
fi
...
if [ ! -s "$TMP_LIST" ]; then
    ...
    enviar_mensaje "❌ Error: No hay hosts válidos en LAB11/LAB12"
    enviar_log
    exit 1
fi
...
enviar_mensaje "⚡ Encendiendo LAB11 (192.168.11.101-146) y LAB12 (192.168.12.101-146) - $hoy_legible"
...
if [ -z "$no_respondieron" ]; then
    enviar_mensaje "✅ LAB11 y LAB12 encendidos correctamente."
else
